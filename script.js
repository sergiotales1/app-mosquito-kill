...
// after that, we created win.html and gameOver.html with a image and a button that will redirect (if he wants) the user to the index.html page which has the attribution to offer 3 different levels, each one level chosen will redirect the user to app.html where the game is. sending a message with window.locate.search that will tell the chosen level by the user, with that level in hands we store it into a variable them send it to the main setInterval delay that creates mosquitos .